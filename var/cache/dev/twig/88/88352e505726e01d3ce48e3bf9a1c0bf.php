...
class __TwigTemplate_f7ec58e82c22551835d8b3d48b4b747a extends Template
{
...
{% block title %}Actualité du jours{% endblock %}

{# {% block content %}
    {% for post in posts %}
        <div class=\"example-wrapper\">
        <h1>{{ post.title }}✅ and id : {{post.id}}</h1>
            {{post.content}}
            {{post.category.name}}
    ...
    {% endfor %}
        <ul style=\"...\">
            {% if totalItemsPost > 10 %}
                {% for page in 1..noOfPages %}
                    <li style=\"display: inline-block\"> 
                    {% if page == currentPage %}
                        <span style=\"color: red\">{{page}}</span>
                    {% else %}
                        <code><a href=\"{{path('app_blog', {'page': page})}}\">{{page}}</a></code>
                    {% endif %}
                    </li>
                {% endfor %}
            {% endif %}
...
{% endblock %} #}
", "blog/index.html.twig", "C:\\Users\\prodigy-188\\Music\\projet1\\templates\\blog\\index.html.twig");
    }
}
